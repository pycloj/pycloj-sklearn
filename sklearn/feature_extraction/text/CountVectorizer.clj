(ns sklearn.feature-extraction.text.CountVectorizer
...
(defn CountVectorizer 
  "Convert a collection of text documents to a matrix of token counts
    ...
    be safely removed using delattr or set to None before pickling.
    "
  [ & {:keys [input encoding decode_error strip_accents lowercase preprocessor tokenizer stop_words token_pattern ngram_range analyzer max_df min_df max_features vocabulary binary dtype]
       :or {input "content" encoding "utf-8" decode_error "strict" lowercase true token_pattern "(?u)\b\w\w+\b" ngram_range (1, 1) analyzer "word" max_df 1.0 min_df 1 binary false dtype <class 'numpy.int64'>}} ]
  
   (py/call-attr-kw text "CountVectorizer" [] {:input input :encoding encoding :decode_error decode_error :strip_accents strip_accents :lowercase lowercase :preprocessor preprocessor :tokenizer tokenizer :stop_words stop_words :token_pattern token_pattern :ngram_range ngram_range :analyzer analyzer :max_df max_df :min_df min_df :max_features max_features :vocabulary vocabulary :binary binary :dtype dtype }))
