(ns sklearn.linear-model.RidgeCV
...
(defn RidgeCV 
  "Ridge regression with built-in cross-validation.
    ...
    RidgeClassifierCV : Ridge classifier with built-in cross validation
    "
  [ & {:keys [alphas fit_intercept normalize scoring cv gcv_mode store_cv_values]
       :or {alphas (0.1, 1.0, 10.0) fit_intercept true normalize false store_cv_values false}} ]
  
   (py/call-attr-kw linear-model "RidgeCV" [] {:alphas alphas :fit_intercept fit_intercept :normalize normalize :scoring scoring :cv cv :gcv_mode gcv_mode :store_cv_values store_cv_values }))
