(ns sklearn.model-selection.GridSearchCV
...
(defn GridSearchCV 
  "Exhaustive search over specified parameter values for an estimator.
    ...
    "
  [estimator param_grid scoring n_jobs & {:keys [iid refit cv verbose pre_dispatch error_score return_train_score]
                       :or {iid "warn" refit true cv "warn" verbose 0 pre_dispatch "2*n_jobs" error_score "raise-deprecating" return_train_score false}} ]
    (py/call-attr-kw model-selection "GridSearchCV" [estimator param_grid scoring n_jobs] {:iid iid :refit refit :cv cv :verbose verbose :pre_dispatch pre_dispatch :error_score error_score :return_train_score return_train_score }))
