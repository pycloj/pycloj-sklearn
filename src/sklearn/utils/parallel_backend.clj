...
(defn parallel-backend 
  "Change the default backend used by Parallel inside a with block.
    ...
    "
  [backend & {:keys [n_jobs inner_max_num_threads]
                       :or {n_jobs -1}} ]
    (py/call-attr-kw utils "parallel_backend" [backend] {:n_jobs n_jobs :inner_max_num_threads inner_max_num_threads }))

(defn unregister 
  ""
  [ self  ]
  (py/call-attr self "unregister"  self  ))
