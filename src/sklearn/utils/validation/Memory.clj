(ns sklearn.utils.validation.Memory
...
(defn Memory 
  " A context object for caching a function's return value each time it
        ...
        backend_options: dict, optional
            ...
            the store backend.
    "
  [ & {:keys [location backend cachedir mmap_mode compress verbose bytes_limit backend_options]
       :or {backend "local" compress false verbose 1}} ]
  
   (py/call-attr-kw validation "Memory" [] {:location location :backend backend :cachedir cachedir :mmap_mode mmap_mode :compress compress :verbose verbose :bytes_limit bytes_limit :backend_options backend_options }))
...
(defn cache 
  ...
  [self & {:keys [func ignore verbose mmap_mode]
                       :or {mmap_mode false}} ]
    (py/call-attr-kw validation "cache" [] {:func func :ignore ignore :verbose verbose :mmap_mode mmap_mode }))

(defn cachedir 
  ""
  [ self ]
    (py/call-attr validation "cachedir"  self))
...
(defn clear 
  ...
  [self & {:keys [warn]
                       :or {warn true}} ]
    (py/call-attr-kw validation "clear" [] {:warn warn }))

(defn debug 
  ""
  [self  & {:keys [msg]} ]
    (py/call-attr-kw validation "debug" [self] {:msg msg }))
...
(defn eval 
  " Eval function func with arguments `*args` and `**kwargs`,
        ...
        "
  [self  & {:keys [func]} ]
    (py/call-attr-kw validation "eval" [self] {:func func }))

(defn format 
  "Return the formatted representation of the object."
  [self & {:keys [obj indent]
                       :or {indent 0}} ]
    (py/call-attr-kw validation "format" [] {:obj obj :indent indent }))

(defn reduce-size 
  "Remove cache elements to make cache size fit in ``bytes_limit``."
  [ self ]
  (py/call-attr validation "reduce_size"  self ))

(defn warn 
  ""
  [self  & {:keys [msg]} ]
    (py/call-attr-kw validation "warn" [self] {:msg msg }))
